...
def emit_framing_check(
    ...
    emit("    _bb_min = mathutils.Vector((1e9, 1e9, 1e9))")
    emit("    _bb_max = mathutils.Vector((-1e9, -1e9, -1e9))")
    ...
    emit("                _bb_min.x = min(_bb_min.x, _wpt.x)")
    emit("                _bb_min.y = min(_bb_min.y, _wpt.y)")
    emit("                _bb_min.z = min(_bb_min.z, _wpt.z)")
    emit("                _bb_max.x = max(_bb_max.x, _wpt.x)")
    emit("                _bb_max.y = max(_bb_max.y, _wpt.y)")
    emit("                _bb_max.z = max(_bb_max.z, _wpt.z)")
    ...
    emit("        # True 3D bounding box centroid for accurate aim")
    emit("        _center = (_bb_min + _bb_max) / 2")
    ...
    emit("        # Re-aim at bounding box centroid")
