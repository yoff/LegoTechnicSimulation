...
def generate_blender_script(
    ...
    emit("")
    emit("def _check_and_adjust_framing(margin=0.03):")
    emit("    \"\"\"Ensure all units stay within camera frame across all frames.\"\"\"")
    emit("    cam = scene.camera")
    emit("    if cam is None:")
    emit("        print('Framing check: no active camera, skipping.')")
    emit("        return")
    emit("    frame_start = scene.frame_start")
    emit("    frame_end = scene.frame_end")
    emit("    global_min_x, global_min_y = 1.0, 1.0")
    emit("    global_max_x, global_max_y = 0.0, 0.0")
    emit("    extreme_frames = []")
    emit("    for f in range(frame_start, frame_end + 1):")
    emit("        scene.frame_set(f)")
    emit("        frame_min_x, frame_min_y = 1.0, 1.0")
    emit("        frame_max_x, frame_max_y = 0.0, 0.0")
    emit("        for obj in _units:")
    emit("            if obj is None or obj.type != 'MESH':")
    emit("                continue")
    emit("            bbox = obj.bound_box")
    emit("            for corner in bbox:")
    emit("                world_pt = obj.matrix_world @ mathutils.Vector(corner)")
    emit("                co = world_to_camera_view(scene, cam, world_pt)")
    emit("                frame_min_x = min(frame_min_x, co.x)")
    emit("                frame_min_y = min(frame_min_y, co.y)")
    emit("                frame_max_x = max(frame_max_x, co.x)")
    emit("                frame_max_y = max(frame_max_y, co.y)")
    emit("        if frame_min_x < margin or frame_min_y < margin or frame_max_x > 1 - margin or frame_max_y > 1 - margin:")
    emit("            extreme_frames.append(f)")
    emit("        global_min_x = min(global_min_x, frame_min_x)")
    emit("        global_min_y = min(global_min_y, frame_min_y)")
    emit("        global_max_x = max(global_max_x, frame_max_x)")
    emit("        global_max_y = max(global_max_y, frame_max_y)")
    emit("    print(f'Framing check: screen bounds X=[{global_min_x:.3f}, {global_max_x:.3f}] Y=[{global_min_y:.3f}, {global_max_y:.3f}]')")
    emit("    if extreme_frames:")
    emit("        print(f'  Extreme frames ({len(extreme_frames)}): {extreme_frames[:10]}{\"+...\" if len(extreme_frames) > 10 else \"\"}')")
    emit("    # Check if adjustment is needed")
    emit("    needs_adjust = (global_min_x < margin or global_min_y < margin or")
    emit("                    global_max_x > 1 - margin or global_max_y > 1 - margin)")
    emit("    if not needs_adjust:")
    emit("        print('  Framing OK – all content within margin.')")
    emit("        scene.frame_set(frame_start)")
    emit("        return")
    emit("    # Compute scale factor to fit content with margin")
    emit("    content_width = global_max_x - global_min_x")
    emit("    content_height = global_max_y - global_min_y")
    emit("    target_width = 1.0 - 2 * margin")
    emit("    target_height = 1.0 - 2 * margin")
    emit("    scale_x = content_width / target_width if target_width > 0 else 1.0")
    emit("    scale_y = content_height / target_height if target_height > 0 else 1.0")
    emit("    scale_factor = max(scale_x, scale_y)")
    emit("    if scale_factor <= 1.0:")
    emit("        # Content fits but is off-center; for now just report")
    emit("        print(f'  Content fits but may be off-center (scale={scale_factor:.3f}).')")
    emit("        scene.frame_set(frame_start)")
    emit("        return")
    emit("    # Apply adjustment")
    emit("    if cam.data.type == 'ORTHO':")
    emit("        old_scale = cam.data.ortho_scale")
    emit("        cam.data.ortho_scale *= scale_factor")
    emit("        print(f'  Adjusted ortho_scale: {old_scale:.4f} -> {cam.data.ortho_scale:.4f} (x{scale_factor:.3f})')")
    emit("    else:")
    emit("        # Perspective: pull camera back along its local -Z axis")
    emit("        pull_back = scale_factor - 1.0")
    emit("        view_dir = cam.matrix_world.to_quaternion() @ mathutils.Vector((0, 0, -1))")
    emit("        old_loc = cam.location.copy()")
    emit("        cam.location -= view_dir * (pull_back * cam.location.length * 0.5)")
    emit("        print(f'  Adjusted camera location: pulled back by {pull_back:.3f} (factor {scale_factor:.3f})')")
    emit("        print(f'    {old_loc} -> {cam.location}')")
    emit("    scene.frame_set(frame_start)")
    emit("")
    emit("_check_and_adjust_framing()")
    emit("print()")
